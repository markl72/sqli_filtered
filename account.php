	...
	try{
		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_assoc()) {
        			echo " <p>Name: " . htmlentities($row["last_name"]). ", " . htmlentities($row["first_name"]). " <BR>Address: " . htmlentities($row["address"]). "<br>Phone number: " . htmlentities($row["phone_no"]) . "</p>";
    			}
		}
		else
		{
		...
		echo "<p><br><br><br><br><font color=\"red\">" . htmlentities($sql) . "</font></p>";
	}
